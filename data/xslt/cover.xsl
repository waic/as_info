...
<h1>アクセシビリティ・サポーテッド（AS）情報：2014年6月版</h1>

<ul>
<li>公開日：2014年7月4日</li>
<li>作成者：ウェブアクセシビリティ基盤委員会（WAIC）実装ワーキンググループ（WG2）</li>
<li>前のバージョン：<a href="/docs/jis2010-as-understanding/201403/">2014年3月版（2014年3月31日公開）</a></li>
</ul>

<h2>1. はじめに</h2>

<p>この文書は、アクセシビリティ・サポーテッド情報に関する解説文書である。ウェブコンテンツの設計・開発に関わる者が、それぞれの実装方法がアクセシビリティ・サポーテッドであるかどうか、つまり達成基準を満たすことのできる実装方法であるかどうかを判断するための参考に用いることができる。</p>

<h2>2. 「アクセシビリティ・サポーテッド」の定義</h2>

<p>JIS X 8341-3:2010では、「6.2 設計」の「b) 使用するウェブコンテンツ技術及び実装方法」で次のような要求事項がある。</p>
<blockquote class="box1">
<p>（略）箇条7 の達成基準を満たすためには，使用するウェブコンテンツ技術及び実装方法が実際に利用者にとって利用可能であることを確認しなければならない。例えば，仕様上は定められているがユーザエージェント（ウェブブラウザ，支援技術など）がサポートしていない方法で実装しても，達成基準を満たしているとはいえな
い。</p>
<p>使用するウェブコンテンツ技術の実装方法が達成基準を満たすことができるかどうかを確認することは，設計・開発する者の責任である。</p>
</blockquote>
<p>つまり、「アクセシビリティ・サポーテッド」であるというのは、言い換えると「実際に利用者にとって利用可能」であるという意味である。「アクセシビリティ・サポーテッド」の用語定義や詳細な解説は、JIS X 8341-3:2010の規格票や「WCAG 2.0 
...
<p>また、「使用するウェブコンテンツ技術の実装方法が達成基準を満たすことができるかどうかを確認することは，設計・開発する者の責任である。」とある。それを確認することをできるかぎり容易にするために、ウェブアクセシビリティ基盤委員会（WAIC）では、テストファイルを作成し、ブラウザや支援技術による独自の検証を行って、その結果を公開している。なお、これらの検証結果について、各実装方法の提供者は何の責務も負わない。</p>
...
適用する達成基準」で選択した達成基準の意図を正しく理解することが重要である。その達成基準がどのようなユーザーに対してどのようなメリットを提供するものであるかを理解していなければ、適切な実装方法を選ぶことはできないからである。</p>
<p>各達成基準の意図を理解するには、「WCAG 2.0 解説書」を参照するとよい。「WCAG 2.0 解説書」では、それぞれの達成基準の意図を解説しており、その達成基準を満たすことによってどのようなユーザーがどのようなメリットを得ることができるかについて確認することができる。</p>

<h4>例：「7.2.4.1 ブロックスキップに関する達成基準」（等級A）の場合</h4>

<p>例えば、「7.2.4.1 ブロックスキップに関する達成基準」の意図を確認する際には、<a href="http://waic.jp/docs/UNDERSTANDING-WCAG20/navigation-mechanisms-skip.html">WCAG 2.0 解説書「ブロック・スキップ：達成基準 2.4.1 を理解する」</a>を参照する。「この達成基準の意図」と「達成基準 2.4.1 の具体的なメリット」を読むと、この達成基準は、主にスクリーンリーダーの利用者、キーボード又はキーボード・インタフェースだけを使用している利用者、そして画面拡大ソフトを使用している利用者が、ウェブページのメインコンテンツへ素早くかつ容易に到達できるようにすることを意図していることがわかる。</p>

<h3>3-2. 各実装方法がアクセシビリティ・サポーテッドであるかどうかを判断する</h3>

<p>確認した達成基準の意図をふまえて、その達成基準を満たすことのできる実装方法を選択しなければならない。達成基準を満たすことのできる実装方法は、その多くが「WCAG 2.0 解説書」で挙げられている。ただし、「WCAG 2.0 解説書」の「達成基準を満たすことのできる実装方法」で挙げられている実装方法は、必ずしも全てがアクセシビリティ・サポーテッドであるとはいえないことに注意しなければならない。</p>
<p>実装方法を選択するにあたっては、「アクセシビリティ・サポーテッド検証結果」を参照して、各実装方法の検証結果を確認し、アクセシビリティ・サポーテッドであるかどうかを判断する。なお、「WCAG 2.0 解説書」の「達成基準を満たすことのできる実装方法」で挙げられている実装方法の中にはブラウザや支援技術によるサポート状況を考慮する必要がないものも含まれており、それらは検証の対象外となっている。そのため、「WCAG 2.0 解説書」で挙げられている実装方法のうち、検証結果の一覧表にない実装方法については、アクセシビリティ・サポーテッドであるかどうかを判断する必要は低いと考えられる。</p>

<h2>4. 判断する際の注意点</h2>

<h3>4-1. 最新の「アクセシビリティ・サポーテッド検証結果」を参照する</h3>

<p>「<a href="http://waic.jp/docs/jis2010/as.html">アクセシビリティ・サポーテッド検証結果</a>」は、必要に応じて随時更新されていく予定である。アクセシビリティ・サポーテッドであるかどうかを判断する際には、その時点での最新の検証結果を参照すること。なお、本文書を作成している時点での最新版は2014年6月版である。</p>
...
<li>英語圏：<a href="http://webaim.org/projects/screenreadersurvey5/">Screen Reader User Survey #5 Results（WebAIM）</a></li>
</ul>
</dd>
</dl>
...
<p>ウェブアクセシビリティ基盤委員会（WAIC）実装ワーキンググループ（WG2）では、「<a href="http://waic.jp/docs/jis2010/as.html">アクセシビリティ・サポーテッド検証結果 2014年6月版</a>」をもとに、それぞれの達成基準を満たすことのできる実装方法についての解説を作成した。なお、検証結果において全てのブラウザおよび支援技術によってサポートされている実装方法（前述の「4-2. 検証結果が全て「○」となっている実装方法」を参照）は、解説を作成していない。</p>
...
<p>2014年3月版と2014年6月版の間の変更点は以下の通りである。</p>

<h4>見解の変更</h4>

<p>「アクセシビリティ・サポーテッド検証結果」2014年6月版に基づいて見解の見直しを実施した。見直しの結果、見解を変更した項目を以下に示す。</p>

<h5>7.2.4.1 ブロックスキップに関する達成基準</h5>
<table>
<tr>
<th>テストファイルNo.</th>
<th>2014年3月版での見解</th>
...
<h5>7.1.1.1 非テキストコンテンツに関する達成基準</h5>
<table>
<tr>
<th>テストファイルNo.</th>
<th>2012年9月版での見解</th>
...
<tr>
<td><a href="H37-2.html">H37-2</a></td>
<td class="ok">達成可能</td>
<td class="warn">要注意</td>
</tr>
<tr>
<td><a href="H53-1.html">H53-1</a></td>
<td class="ng">達成不可能</td>
<td class="warn">要注意</td>
</tr>
<tr>
<td><a href="H53-2.html">H53-2</a></td>
<td class="ng">達成不可能</td>
<td class="warn">要注意</td>
</tr>
</table>

<h5>7.1.3.1 情報及び関係性に関する達成基準</h5>
...
<tr>
<td><a href="SCR26.html">SCR26</a></td>
<td class="warn">要注意</td>
<td class="ok">達成可能</td>
</tr>
</table>
...
<tr>
<td><a href="H64-1.html">H64-1</a></td>
<td class="ng">達成不可能</td>
<td class="warn">要注意</td>
</tr>
<tr>
<td><a href="H64-2.html">H64-2</a></td>
<td class="ng">達成不可能</td>
<td class="warn">要注意</td>
</tr>
<tr>
<td><a href="H69.html">H69</a></td>
<td class="warn">要注意</td>
<td class="ok">達成可能</td>
</tr>
<tr>
<td><a href="SCR28-1.html">SCR28-1</a></td>
<td class="ng">達成不可能</td>
<td class="warn">要注意</td>
</tr>
<tr>
<td><a href="SCR28-2.html">SCR28-2</a></td>
<td class="ng">達成不可能</td>
<td class="warn">要注意</td>
</tr>
</table>

<h5>7.2.4.3 フォーカス順序に関する達成基準</h5>
...
<h5>7.4.1.2 プログラムで解釈可能な識別名，役割及び設定可能な値に関する達成基準</h5>
...
<h5>7.1.4.4 テキストのサイズ変更に関する達成基準</h5>
...
<tr>
<td><a href="G179-1a.html">G179-1a</a></td>
<td class="ng">達成不可能</td>
<td class="ok">達成可能</td>
</tr>
<tr>
<td><a href="G179-1b.html">G179-1b</a></td>
<td class="ng">達成不可能</td>
<td class="ok">達成可能</td>
</tr>
</table>
